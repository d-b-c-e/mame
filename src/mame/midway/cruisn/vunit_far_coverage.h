...
struct Input { uint32_t far_limit=0;std::array<uint32_t,4> words{}; };
...
{
    depths={};if(input.far_limit!=240000)return false;
    ...
    for(unsigned i=0;i<4;++i) {
        ...
        inside+=values[i]<input.far_limit;
    }
...
inline bool coverage(const std::array<Point,4> &xy,const std::array<double,4> &z,
    double far_limit,Mask &output)
{
    output=Mask{};
    if(!std::isfinite(far_limit) || far_limit<=0 || far_limit>1e9)return false;
    unsigned inside=0;
    for(unsigned i=0;i<4;++i) {
        if(!std::isfinite(z[i]) || z[i]<=0 || z[i]>1e9)return false;
        for(float v:xy[i])if(!std::isfinite(v) || std::abs(v)>65536.f)return false;
        inside+=z[i]<far_limit;
    }
    ...
    for(unsigned i=0;i<4;++i) {
        ...
        if((z[j]<far_limit)!=(z[i]<far_limit)) {
            ...
            const double t=(1.0/far_limit-1.0/z[j])/(1.0/z[i]-1.0/z[j]);
        ...
        if(z[i]<far_limit && !append(xy[i][0],xy[i][1]))return false;
    }
