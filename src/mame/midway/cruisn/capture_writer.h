...
public:
    ...
    struct Stats {
        ...
        uint64_t peak_bytes=0, write_total_us=0, write_max_us=0, drain_us=0;
    };
    ...
    // Nonblocking admission. Saturation is an explicit failed diagnostic, never
    // a reason to block or fall back from the game's renderer. In-flight storage
    // is included in the byte/job budgets, not just the waiting deque.
    bool submit(Request &&request)
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        if (m_stop || request.bitmap.empty() || request.bitmap.size()>m_limit-m_bytes || m_jobs>=32) {
    ...
    Stats finish()
    {
        ...
        { std::lock_guard<std::mutex> lock(m_mutex); m_stop=true; m_ready.notify_one(); }
