...
struct Projection
{
    float scale=0,uvscale=0,clip=0,ox=0,oy=0,focal=0;
};
inline bool prepare_projection(const Context &ctx,Projection &p)
{
    const int vs=int(ctx.regs[0x66])-0x8e,us=int(ctx.regs[0x68])-0x9d;
    if(vs<-31 || vs>31 || us<-31 || us>31 || ctx.regs[0x6c]>30)return false;
    p.scale=std::ldexp(1.0f,vs);p.uvscale=std::ldexp(1.0f,us);
    p.clip=word_float(ctx.regs[0x78]);p.ox=word_float(ctx.regs[0x6a]);p.oy=word_float(ctx.regs[0x6b]);
    p.focal=float(1U<<ctx.regs[0x6c]);
    return std::isfinite(p.clip) && std::isfinite(p.ox) && std::isfinite(p.oy);
}
inline int project_prepared(const uint32_t *d,uint32_t tex,const Context &ctx,const Projection &p,Quad &quad)
{
    const float scale=p.scale,uvscale=p.uvscale,clip=p.clip,ox=p.ox,oy=p.oy;
    ...
    for(unsigned i=0;i<count;++i)
    {
        ...
        const float ooz=p.focal/(v[2]+2.0f);
...
{
    Projection p;
    return prepare_projection(ctx,p)?project_prepared(d,tex,ctx,p,quad):-1;
}
inline bool decode(const std::vector<uint32_t> &words,Context context,Result &result)
{
    // Reuse bounded scratch storage across instances, never geometry or state.
    result.quads.clear();
    result.polygons=result.near_rejected=result.backfaces=result.near_clipped=result.register_writes=0;
    ...
    uint32_t tex=0;Projection projection;bool projection_ready=false;
    for(size_t i=0;i<words.size();)
    {
        ...
        {
            ++result.polygons;
            if(!projection_ready){if(!prepare_projection(context,projection))return false;projection_ready=true;}
            Quad quad;const int status=project_prepared(d,tex,context,projection,quad);
