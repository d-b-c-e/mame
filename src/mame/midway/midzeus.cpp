...
private:
	...
	cruisn::DiagnosticJournal m_endpoint_log,m_endpoint_inputs;
	...
	cruisn::DiagnosticJournal m_endpoint_early_log;
	...
	bool lifetime_scope();
	void lifetime_exit();
	void lifetime_emit(char event,uint32_t slot,uint64_t generation,uint64_t owner,
		const cruisn::scenery_lifetimes::Key &key,uint32_t reason,uint32_t flags);
	bool lifetime_slot(uint32_t slot)const;
	cruisn::scenery_lifetimes::Registry m_lifetimes;
	struct LifetimePending {
		unsigned kind=0;uint32_t slot=0,old_head=0,new_head=0,count=0,base=0;
		double time=0,head_time=0;
	} m_lifetime_pending;
	struct LifetimeOwner {
		cruisn::scenery_lifetimes::Handle handle;uint64_t serial=0;
		bool faded=false,opaque=false;
	};
	...
	memory_passthrough_handler m_lifetime_head_tap,m_lifetime_count_tap,m_lifetime_reset_tap;
	memory_passthrough_handler m_lifetime_owner_tap,m_lifetime_ready_tap,m_lifetime_submit_tap;

	void scene_observer_start();
	...
	cruisn::DiagnosticJournal m_scene_log;
	...
	cruisn::DiagnosticJournal m_waiting_log;
	...
	cruisn::DiagnosticJournal m_handover_log,m_handover_cohorts;
	...
	cruisn::DiagnosticJournal m_compose_log;
	...
	cruisn::DiagnosticJournal m_scene_material_log;
	...
	cruisn::DiagnosticJournal m_scene_fence_log;
	...
	double m_active_seal_us=0;
	unsigned m_active_seal_pages=0;bool m_active_seal_verified=false;
	cruisn::exotica_scene::Parameters m_active_seed;
	uint64_t m_active_seed_scene=0,m_active_scenes=0,m_active_quads=0;
	uint32_t m_active_seed_bank=0;
	std::set<uint32_t> m_active_submitted,m_active_snapshots;
	memory_passthrough_handler m_active_submission_tap;
	cruisn::DiagnosticJournal m_active_log;
...
uint32_t crusnexo_state::scene_read(uint32_t address)
{
	address&=0xffffff;
	if(address<0x40000)return m_ram_base[address];
	if(address>=0xa00000 && address<0xc00000)
		return reinterpret_cast<const uint32_t *>(memregion("maindata")->base())[address-0xa00000];
	if(address>=0xc00000)
		return reinterpret_cast<const uint32_t *>(m_mainbank->base())[address-0xc00000];
	if(address>=0x87fe00 && address<0x880000)
	{
		auto disabler=machine().disable_side_effects();
		return m_maincpu->space(AS_PROGRAM).read_dword(address);
	}
	fatalerror("Exotica host read outside captured RAM/ROM contract: %08x\n",address);
}

// Explicit read-only original allocation/submission observation. The reusable
// registry carries identities only; this driver adapter checks real guest code,
// free-list/count transactions, reset rebuilding and original model commits.
bool crusnexo_state::lifetime_slot(uint32_t slot)const
{
	return slot>=0x1000 && uint64_t(slot)+31<=0x40000 && (slot+31<=0x30000 || slot>=0x32000);
}
	...
	const cruisn::scenery_lifetimes::Key &key,uint32_t reason,uint32_t flags)
{
	...
	if(m_lifetime_log.print("%c,%u,%.17g,%llu,%llu,%llu,%u,%llu,%llu,%u,%u,%u,%u\n",
		...
		(unsigned long long)generation,slot,(unsigned long long)owner,(unsigned long long)key.realm,
...
bool crusnexo_state::lifetime_scope()
{
	...
	if(!m_lifetime_started) {
		...
		m_lifetime_started=true;
		lifetime_emit('L',m_lifetime_initial_head,0,0,{},m_lifetime_initial_count,0);
	}
	return true;
}
...
void crusnexo_state::lifetime_start()
{
	...
	auto number=[](const char *name) {
		...
		char *end=nullptr;const auto n=strtoul(value,&end,10);
		if(*end || n<1799 || n>15999)fatalerror("Invalid Exotica lifetime range %s\n",name);
		return uint32_t(n);
	};
	...
	if(m_lifetime_log.print("event,frame,time,sequence,epoch,generation,slot,owner,realm,section,source,reason,flags\n")<0)
		fatalerror("Cannot write Exotica lifetime header\n");
	auto &space=m_maincpu->space(AS_PROGRAM);
	m_lifetime_head_tap=space.install_write_tap(0x10a8,0x10a8,"exotica_lifetime_head",
		[this](offs_t,uint32_t &data,uint32_t mask) {
			if(!lifetime_scope())return;
			if(mask!=UINT32_MAX)fatalerror("Exotica lifetime partial head write\n");
			auto &p=m_lifetime_pending;const auto pc=m_maincpu->state_int(TMS320C3X_PC);
			if(p.kind==3) {
				if(pc!=0xbbcd || p.head_time || data!=p.base || m_ram_base[0x10a8]!=p.old_head || m_ram_base[0x10a9]!=1200)
					fatalerror("Exotica lifetime reset head\n");
				p.head_time=machine().time().as_double();return;
			}
			if(p.kind)fatalerror("Exotica lifetime overlapping transaction\n");
			...
			if(pc==0x85b4) {
				...
				p.kind=2;p.slot=uint32_t(m_maincpu->state_int(TMS320C3X_AR2));
				if(!lifetime_slot(p.slot) || p.slot!=data || m_ram_base[p.slot]!=p.old_head)
					fatalerror("Exotica lifetime removal link\n");
			} else fatalerror("Exotica lifetime unmapped head PC%x\n",unsigned(pc));
		});
	m_lifetime_count_tap=space.install_write_tap(0x10a9,0x10a9,"exotica_lifetime_count",
		[this](offs_t,uint32_t &data,uint32_t mask) {
			if(!lifetime_scope())return;
			...
			if(!p.kind && pc==0xbbc9) {
				...
				if(data!=1200 || m_ram_base[0xbbba]!=0x10a8 || !lifetime_slot(p.base) || uint64_t(p.base)+1201*31>0x40000 || (p.base<0x32000 && uint64_t(p.base)+1201*31>0x30000))
					fatalerror("Exotica lifetime reset operands\n");
				m_lifetime_reset_tap=m_maincpu->space(AS_PROGRAM).install_write_tap(p.base,p.base+1201*31,"exotica_lifetime_reset_tail",
					[this](offs_t address,uint32_t &value,uint32_t bits) {
						if(machine().side_effects_disabled() || m_maincpu->state_int(TMS320C3X_PC)!=0xbbd5)return;
						auto &p=m_lifetime_pending;
						if(bits!=UINT32_MAX || p.kind!=3 || !p.head_time || value || address!=p.base+1200*31 ||
							address!=uint32_t(m_maincpu->state_int(TMS320C3X_AR0)) || m_ram_base[0x10a8]!=p.base || m_ram_base[0x10a9]!=1200)
							fatalerror("Exotica lifetime reset tail\n");
						for(unsigned i=0;i<1200;++i)if(m_ram_base[p.base+i*31]!=p.base+(i+1)*31)fatalerror("Exotica lifetime reset link\n");
			...
			const auto expected=p.kind==1?0xbbfa:0xbc68;
			if((p.kind!=1 && p.kind!=2) || pc!=expected || m_ram_base[0x10a8]!=p.new_head || m_ram_base[0x10a9]!=p.count ||
				data!=(p.kind==1?p.count-1:p.count+1) || data>4096)
	...
	m_lifetime_submit_tap=space.install_write_tap(0x46e,0x46e,"exotica_lifetime_model_commit",
		[this](offs_t,uint32_t &data,uint32_t mask) {
			...
			if(mask!=UINT32_MAX || !lifetime_slot(slot) || data<0x30000 || data>=0x32000 || data!=uint32_t(m_maincpu->state_int(TMS320C3X_AR5)))
				fatalerror("Exotica lifetime original commit bounds\n");
			++m_lifetime_owned;auto &owner=found->second;bool first=false;
			if(!m_lifetimes.submitted(owner.handle,uint64_t(m_screen->frame_number())+1,first))fatalerror("Exotica lifetime stale original commit\n");
			const uint32_t flags=uint32_t(m_maincpu->state_int(TMS320C3X_R6));uint32_t reason=first?1:0;
			m_lifetime_first_draws+=first;
...
void crusnexo_state::endpoint_commit(uint32_t end,uint32_t flags,const LifetimeOwner &owner)
{
	...
	if(m_endpoint_admit_log) {
		...
		if(m_endpoint_admit_log.print("%llu,%u,%llu,%u,%llu,%u,%llu,%llu\n",(unsigned long long)id,unsigned(p.admitted),
...
void crusnexo_state::endpoint_model(uint32_t base,uint32_t count,uint32_t yscale)
{
	...
	if(snapshot) {
		...
		if(m_endpoint_inputs.put('\n')==EOF || m_endpoint_inputs.tell()<0 || m_endpoint_inputs.tell()>64*1024*1024)
	...
	if(m_endpoint_log.print("%llu,%u,%.17g,%u,%.17g,%llu,%llu,%u,%llu,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u\n",
...
void crusnexo_state::endpoint_exit()
{
	...
	if(m_endpoint_early_log) {
		bool good=!m_endpoint_early_log.error();if(m_endpoint_early_log.close())good=false;
	...
	if(m_endpoint_admit_log) {
		bool good=!m_endpoint_admit_log.error() && !m_endpoint_admit_packets.error();
		if(m_endpoint_admit_log.close())good=false;
		if(m_endpoint_admit_packets.close())good=false;
...
void crusnexo_state::scene_observer_start()
{
	...
	auto number=[](const char *name,uint32_t low,uint32_t high,uint32_t fallback) {
		const char *text=std::getenv(name);if(!text)return fallback;
		char *end=nullptr;const auto n=strtoul(text,&end,10);
		if(!text[0] || *end || n<low || n>high)fatalerror("Invalid Exotica host bound: %s\n",name);
		return uint32_t(n);
	};
	...
	if(!m_scene_first || m_scene_last<m_scene_first || m_scene_last-m_scene_first>10000 ||
		...
		fatalerror("Exotica host observation requires bounded frames and legacy Zeus policy\n");
	if(const char *text=std::getenv("MIDZ_HOST_SNAPSHOTS"))
	{
		while(*text) {
			char *end=nullptr;const auto n=strtoul(text,&end,10);
			if(end==text || (*end && *end!=',') || n<m_scene_first || n>m_scene_last ||
				!m_scene_snapshots.insert(uint32_t(n)).second || m_scene_snapshots.size()>16)
				fatalerror("Invalid Exotica host snapshot interval\n");
			text=*end?end+1:end;
	...
	if(m_active_mode) {
		...
		m_zeus->midz_active_wave_enable();
		m_active_submission_tap=space.install_write_tap(0x46e,0x46e,"exotica_active_original_submission",
			[this](offs_t offset,uint32_t &data,uint32_t mask) {
				if(machine().side_effects_disabled() || !m_scene_open || !m_active_capture.lists() ||
					m_maincpu->state_int(TMS320C3X_PC)!=0x6970)return;
				const uint32_t object=uint32_t(m_maincpu->state_int(TMS320C3X_AR7));
				if(object<0x1000 || object+31>0x40000 || data<0x30000 || data>=0x32000 ||
					m_active_submitted.size()>=4096)fatalerror("Exotica active original-submission bounds\n");
				m_active_submitted.insert(object);
			});
		...
		fprintf(stderr,"MIDZ_HOST_ACTIVE_SEAL_PAGES=1\n");
	}
	...
	m_scene_marker_tap=space.install_write_tap(0xff2,0xff2,"exotica_host_scene_begin",
		[this](offs_t offset,uint32_t &data,uint32_t mask) {
			...
			if(m_active_mode) {
				...
				m_active_capture.clear();m_active_submitted.clear();m_active_seed_scene=0;m_active_sealed_scene=0;
			}
...
void crusnexo_state::scene_observer_prepare()
{
	...
	if(read(0x6963)!=0x0820b47d || read(0xb47d)!=0x24860000 || read(0x67bf)!=0x87ff35 ||
	...
	for(unsigned i=0;i<46;++i)p.setup.commands[i]=read(0xb479+i);
	const uint32_t program_addresses[]={0xe7c1,0xe7c7,0xe7d3,0xe7d9};
	for(unsigned i=0;i<4;++i) {
		p.setup.programs[i]=read(program_addresses[i]);
		for(unsigned j=0;j<4;++j)p.setup.bodies[i][j]=read(p.setup.programs[i]+j);
	}
	const uint32_t defaults=read(0xe4),size=read(defaults)+1;
	if(!size || size>16)fatalerror("Exotica host default state bounds\n");
	for(unsigned i=0;i<size;++i)p.setup.defaults.push_back(read(defaults+1+i));
	std::array<uint32_t,3> position;std::array<uint32_t,9> rotation;
	std::copy_n(p.setup.object.begin()+1,3,position.begin());std::copy_n(p.setup.object.begin()+5,9,rotation.begin());
	cruisn::exotica_transform::Prepared transformed;
	if(!cruisn::exotica_transform::prepare(position,p.camera,p.view,rotation,p.alternate,flags,transformed))
		fatalerror("Exotica host original transform\n");
	const auto packet=cruisn::exotica_transform::packet(transformed,p.scale,false);
	for(unsigned i=0;i<3;++i)pending.translation[i]=cruisn::zeus_state::floating(packet[i+1]);
	const uint32_t model=p.setup.object[17];
	if(!cruisn::exotica_future::span(model,6))fatalerror("Exotica host original model pointer\n");
	const uint32_t selected=cruisn::exotica_transform::select_model(model,read(model),transformed.depth);
	if(!cruisn::exotica_future::span(selected,6))fatalerror("Exotica host original LOD pointer\n");
...
void crusnexo_state::scene_observer_model(uint32_t base,uint32_t count,uint32_t yscale)
{
	if(!m_scene_log || m_scene_pending.empty())return;
	const double now=machine().time().as_double();
	auto found=m_scene_pending.end();
	for(auto it=m_scene_pending.begin();it!=m_scene_pending.end();++it) {
		if(now-it->time>.0176)fatalerror("Exotica host original model join expired\n");
		if(it->base==base && it->count==count && now>=it->time &&
			std::equal(it->translation.begin(),it->translation.end(),m_zeus->zeus_trans)) {
			if(found!=m_scene_pending.end())fatalerror("Exotica host ambiguous original model join\n");
			found=it;
		}
	}
	...
	for(unsigned i=0;i<46;++i)if(p.setup.commands[i]!=read(0xb479+i))fatalerror("Exotica host setup commands changed\n");
	if(p.scale!=read(0x67db) || p.setup.palette_setup!=read(0x15f2))fatalerror("Exotica host setup scale/palette changed\n");
	auto &c=p.context;const auto &z=*m_zeus;
	c.quad_size=z.zeus_quad_size;c.ucode=z.m_curUCodeSrc;c.palette=z.m_curPalTableSrc;
	c.texture=z.zeus_texbase;c.yscale=yscale;c.zoffset=z.m_useZOffset;p.render_policy=z.m_upstream_render;
	std::memcpy(c.matrix.data(),z.zeus_matrix,sizeof(z.zeus_matrix));
	std::copy_n(z.zeus_trans,4,c.translation.begin());std::copy_n(z.zeus_light,3,c.light.begin());
	std::copy_n(z.m_zeusbase,128,c.regs.begin());std::copy_n(z.m_renderRegs,80,c.render.begin());
	...
	if(m_scene_snapshots.erase(p.frame)) {
		auto dump=[](const std::string &name,const void *data,size_t bytes) {
			...
			const bool ok=fwrite(data,1,bytes,f)==bytes;const int closed=fclose(f);
			if(!ok || closed)fatalerror("Cannot complete Exotica host snapshot\n");
		};
		if(!m_scene_rom_saved) {
			dump("exotica-host-main-rom.bin",memregion("maindata")->base(),0x800000);
			dump("exotica-host-banked-rom.bin",memregion("bankeddata")->base(),0x3000000);m_scene_rom_saved=true;
		}
		const std::string prefix="exotica-host-"+std::to_string(p.frame);
		const auto parameters=cruisn::exotica_scene::parameter_words(p,pending.bank,m_scene_loading!=0);
		const auto instances=cruisn::exotica_scene::instance_words(scene);
	...
	auto us=[](auto a,auto b){return std::chrono::duration<double,std::micro>(b-a).count();};
	if(m_scene_log.print("%u,%u,%.12f,%.12f,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%016llx,%.3f,%.3f,%.3f,%.3f,0,%llu,%u,%.12f,%u,%u,%.3f,%u,%u,%u,%u,%u\n",
...
void crusnexo_state::scene_fence_ready(bool immediate)
{
	...
	if(m_scene_fence_log.print("%llu,%u,%u,%.12f,%u,%.12f,%u,%u,%u,%u,0,%u\n",
...
void crusnexo_state::scene_waiting_ready()
{
	...
	if(m_handover_mode==2) {
		...
		auto us=[](auto a,auto b){return std::chrono::duration<double,std::micro>(b-a).count();};
		if(m_scene_material_log.print("%llu,%u,%llu,0,%u,%u,%016llx,%.3f,%.3f,%.3f,%.3f\n",
...
void crusnexo_state::scene_active_list(uint32_t entry,uint32_t head)
{
	const auto cycles=m_maincpu->total_cycles();
	auto read=[&](uint32_t address){return scene_read(address);};
	const uint32_t code[][2]={{0x681f,0x082fbbb5},{0x6823,0x082fbbb6},{0x682f,0x082fbbb7},{0x6833,0x082fbbb8}};
	for(const auto &word:code)if(read(word[0])!=word[1])fatalerror("Exotica active list code signature\n");
	cruisn::exotica_active::Parameters p;
	p.mode=read(0x75);p.projection_table=read(0x67cc);p.margin=uint32_t(m_scene_margin);
	for(unsigned i=0;i<3;++i) {
		p.camera[i]=read(0xfeb+i);
...
void crusnexo_state::scene_active_seal()
{
	const auto cycles=m_maincpu->total_cycles();const auto started=std::chrono::steady_clock::now();
	auto read=[&](uint32_t address){return scene_read(address);};
	// The game can advance camera/animation after ordinary_end while the old
	...
	const auto pages=m_zeus->midz_active_wave_pages();
	const auto *wave=reinterpret_cast<const uint8_t *>(m_zeus->m_waveram.get());
	for(const auto page:pages) {
		if(page>=4096)fatalerror("Exotica sealed page bounds\n");
		std::memcpy(m_active_wave.data()+page*4096,wave+page*4096,4096);
	}
	m_active_seal_pages=unsigned(pages.size());
	m_active_seal_verified=m_scene_material_pages==2 || m_active_snapshots.count(m_active_seed.frame)!=0;
	if(m_active_seal_verified && std::memcmp(m_active_wave.data(),wave,m_active_wave.size()))
		fatalerror("Exotica sealed written/full images differ\n");
...
{
	const auto cycles=m_maincpu->total_cycles();const auto started=std::chrono::steady_clock::now();
	if(m_active_seed_scene!=m_scene_fence_scene || m_active_sealed_scene!=m_scene_fence_scene || m_active_seed_bank!=(m_disk_asic_jr[5]&3) ||
		m_active_seed.context.render[4]!=m_zeus->m_renderRegs[4] || m_scene_fence.pending() || !m_zeus->midz_fifo_empty())
		fatalerror("Exotica active scene/page/bank/fence ownership changed\n");
	auto read=[&](uint32_t address)->uint32_t {
		if(address<0x40000)return m_active_ram[address];
		if(address>=0x87fe00 && address<0x880000)return m_active_internal[address-0x87fe00];
		// Remaining model descriptors/programs are immutable, bank-checked ROM.
		if(address<0xa00000 || address>=0x1000000)fatalerror("Exotica sealed read bounds\n");
	...
	for(unsigned i=0;i<12;++i)if(p.setup.constants[i]!=read(0x67d0+i))fatalerror("Exotica active completion constants changed\n");
	for(unsigned i=0;i<46;++i)if(p.setup.commands[i]!=read(0xb479+i))fatalerror("Exotica active completion commands changed\n");
	if(p.scale!=read(0x67db) || p.setup.palette_setup!=read(0x15f2))fatalerror("Exotica active completion scale/palette setup changed\n");
	const uint32_t program_addresses[]={0xe7c1,0xe7c7,0xe7d3,0xe7d9};
	for(unsigned i=0;i<4;++i) {
		if(p.setup.programs[i]!=read(program_addresses[i]))fatalerror("Exotica active completion program changed\n");
		for(unsigned j=0;j<4;++j)if(p.setup.bodies[i][j]!=read(p.setup.programs[i]+j))fatalerror("Exotica active completion program body changed\n");
	}
	const uint32_t defaults=read(0xe4);
	...
	for(const auto &list:selected.lists) {
		...
		for(auto instance:part.instances) {
			bool valid=true;
			for(size_t j=0;j<instance.quad_count;++j)
				valid=valid && cruisn::zeus_margin::depth24(part.quads[instance.first_quad+j],p.frame,page);
	...
	if(m_compose) {
		...
		if(m_compose_log.print("%llu,%u,%llu,%llu,%u,%u,%u,%u,%u,%u,%u\n",
	...
	material.frame=p.frame;material.scene=m_scene_fence_scene;material.snapshot=m_active_snapshots.count(p.frame)!=0;
	packet.margin=uint32_t(m_scene_margin);packet.page=page;packet.draw=m_active_mode==2;
	material.rows=std::move(palettes.rows);
	for(size_t i=0;i<scene.instances.size();++i) {
		const auto &instance=scene.instances[i];
		for(size_t j=0;j<instance.quad_count;++j) {
			cruisn::zeus_margin::Quad q;q.polygon=scene.quads[instance.first_quad+j];q.palette=palettes.instance_rows[i];packet.quads.push_back(q);
		}
	}
	const bool staged=m_scene_material_pages ?
		m_scene_material_image->stage_selected_pages(wave,0x1000000,m_zeus->midz_host_wave_pages(),material.wave) :
		m_scene_material_image->stage(wave,0x1000000,material.wave);
	if(!staged)fatalerror("Exotica active private image staging rejected\n");
	if(m_scene_material_pages==2) {
		cruisn::zeus_host::WaveImage::Packet full;std::vector<uint8_t> a,b;
		if(!m_scene_material_image->stage(wave,0x1000000,full) ||
			!cruisn::zeus_host::WaveImage::encode(full,a) || !cruisn::zeus_host::WaveImage::encode(material.wave,b) || a!=b)
			fatalerror("Exotica active written/full pages differ\n");
		++m_scene_material_verified;
	}
	...
	const auto queued=std::chrono::steady_clock::now();
	if(!m_scene_material_image->apply(material.wave))fatalerror("Exotica active producer image commit rejected\n");
	if(m_scene_material_pages)m_zeus->midz_host_wave_commit();
	const auto committed=std::chrono::steady_clock::now();
	auto us=[](auto a,auto b){return std::chrono::duration<double,std::micro>(b-a).count();};
	if(m_scene_material_log.print("%llu,%u,%llu,%u,%u,%u,%016llx,%.3f,%.3f,%.3f,%.3f\n",
		(unsigned long long)material.scene,p.frame,(unsigned long long)material.wave.generation,
		unsigned(material.wave.pages.size()),unsigned(material.rows.size()),unsigned(material_wire.size()),
		(unsigned long long)material.wave.result_hash,us(assembled,staged_at),us(staged_at,encoded),us(encoded,queued),us(queued,committed))<0)
		fatalerror("Exotica active material log write\n");
	if(material.snapshot) {
		auto dump=[](const std::string &name,const void *data,size_t bytes) {
			FILE *f=fopen(name.c_str(),"wb");if(!f)fatalerror("Cannot open Exotica active snapshot\n");
			const bool ok=fwrite(data,1,bytes,f)==bytes;const int closed=fclose(f);if(!ok || closed)fatalerror("Cannot complete Exotica active snapshot\n");
		};
		const auto prefix="exotica-active-"+std::to_string(p.frame);
		const auto parameters=cruisn::exotica_scene::parameter_words(p,m_active_seed_bank,m_active_sealed_loading);
		const auto instances=cruisn::exotica_scene::instance_words(scene);
...
void crusnexo_state::scene_observer_exit()
{
	...
	if(m_handover_log) {
		const bool good=!m_handover_log.error() && !m_handover_cohorts.error();
		const int closed=m_handover_log.close(),cohorts_closed=m_handover_cohorts.close();
	...
	if(m_waiting_log) {
		const bool good=!m_waiting_log.error();const int closed=m_waiting_log.close();
	...
	if(m_active_log) {
		const int closed=m_active_log.close();
		fprintf(stderr,"MIDZ_HOST_ACTIVE_RESULT complete=%u scenes=%llu quads=%llu remaining=%u\n",
			unsigned(!closed && m_active_scenes==m_scene_matched && m_active_snapshots.empty()),
			(unsigned long long)m_active_scenes,(unsigned long long)m_active_quads,unsigned(m_active_snapshots.size()));
	}
	if(m_scene_fence_log) {
		m_zeus->set_midz_fifo_observer({});
		const int closed=m_scene_fence_log.close();
	...
	if(m_scene_material_log) {
		if(m_scene_material_log.close())fatalerror("Exotica material log close\n");
