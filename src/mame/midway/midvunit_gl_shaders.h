...
static const char *MVGL_MIRROR_FS =
	"\n"
	"#version 430\n"
	"uniform int  uScale;       // 1 = exact mode, >1 = quality mode\n"
	"uniform vec2 uCanvas;      // coarse canvas size\n"
	...
	"layout(location=3) out uint outOriginalIndex;\n"
	"layout(location=4) out uint outOriginalMask;\n"
	"void main() {\n"
	" mirror_source();\n"
	" outFadeAlpha=1.0;\n"
	" outOriginalIndex=outIndex;\n"
	" outOriginalMask=outMask;\n"
	"}\n"
	"\n";

static const char *MVGL_MIRROR_CPU_FS =
	"\n"
	"#version 430\n"
	"uniform usampler2D cpuIndex;\n"
	"uniform usampler2D cpuDirty;\n"
...
static const char *MVGL_FADE_FS =
	"\n"
	"#version 430\n"
	"uniform int  uScale;       // 1 = exact mode, >1 = quality mode\n"
	"uniform vec2 uCanvas;      // coarse canvas size\n"
...
static const char *MVGL_FADE_PAL_FS =
	"\n"
	"#version 430\n"
	"uniform usampler2D idxTex;\n"
	"uniform usampler2D palTex;   // 256x128 R32UI - 32768 palette words\n"
	...
	"out vec4 color;\n"
	"\n"
	"uniform int uDistanceFade;\n"
	"uniform usampler2D originalIdxTex;\n"
	"uniform usampler2D originalMaskTex;\n"
	"uniform sampler2D fadeAlphaTex;\n"
	"bool original_covered(ivec2 p, bool foreground) {\n"
	"    uint tag = texelFetch(originalMaskTex, p, 0).r;\n"
	"    return tag != 0u && (!foreground || (tag & 4u) == 0u);\n"
	"}\n"
	"\n"
	"ivec2 original_fill_layer(ivec2 p, bool foreground) {\n"
	"    // Crack fill: the hardware leaves sub-pixel gaps between adjacent quads\n"
	"    // where the page's PREVIOUS frame shows through (authentic, but it\n"
	"    // shimmers). An unwritten pixel is redirected to its nearest written\n"
	"    // neighbour - only when written pixels exist on BOTH sides along some\n"
	"    // axis (a true between-polys crack). One-sided pixels (silhouettes\n"
	"    // against the cleared 16:9 margins) are left untouched.\n"
	"    if (uFillR == 0 || original_covered(p, foreground)) return p;\n"
	"    ivec2 sz = textureSize(originalMaskTex, 0);\n"
	"    int dl = 0, dr = 0, du = 0, dd = 0;\n"
	"    for (int i = 1; i <= uFillR; i++) {\n"
	"        if (dl == 0 && p.x - i >= 0\n"
	"            && original_covered(p - ivec2(i, 0), foreground)) dl = i;\n"
	"        if (dr == 0 && p.x + i < sz.x\n"
	"            && original_covered(p + ivec2(i, 0), foreground)) dr = i;\n"
	"        if (du == 0 && p.y - i >= 0\n"
	"            && original_covered(p - ivec2(0, i), foreground)) du = i;\n"
	"        if (dd == 0 && p.y + i < sz.y\n"
	"            && original_covered(p + ivec2(0, i), foreground)) dd = i;\n"
	"    }\n"
	"    // fine-dither translucency leaves a 1-px checkerboard: all four axis\n"
	"    // neighbours written, all four diagonals unwritten. That is\n"
	"    // translucency, not a crack - never fill it.\n"
	"    if (dl == 1 && dr == 1 && du == 1 && dd == 1\n"
	"        && p.x > 0 && p.y > 0 && p.x + 1 < sz.x && p.y + 1 < sz.y\n"
	"        && !original_covered(p + ivec2( 1,  1), foreground)\n"
	"        && !original_covered(p + ivec2( 1, -1), foreground)\n"
	"        && !original_covered(p + ivec2(-1,  1), foreground)\n"
	"        && !original_covered(p + ivec2(-1, -1), foreground)) return p;\n"
	"    int wh = (dl > 0 && dr > 0) ? dl + dr : 1 << 20;\n"
	"    int wv = (du > 0 && dd > 0) ? du + dd : 1 << 20;\n"
	"    if (min(wh, wv) >= (1 << 20)) {\n"
	"        // not a crack. Margin extend: the 16:9 margins show black holes\n"
	...
	"        // (vertices carry a +0.5 coarse offset), so probe a short inward\n"
	"        // run rather than the exact edge column.\n"
	"        if (uMargin > 0 && (!foreground || texelFetch(originalMaskTex, p, 0).r == 0u)) {\n"
	"            if (p.x < uMargin + 8) {\n"
	"                int start = max(p.x + 1, uMargin);\n"
	"                for (int k = start; k <= uMargin + 8; k++) {\n"
	"                    ivec2 q = ivec2(k, p.y);\n"
	"                    if (original_covered(q, foreground)) return q;\n"
	"                }\n"
	"            } else if (p.x >= sz.x - uMargin - 8) {\n"
	"                int start = min(p.x - 1, sz.x - 1 - uMargin);\n"
	"                for (int k = start; k >= sz.x - 1 - uMargin - 8; k--) {\n"
	...
	"}\n"
	"\n"
	"ivec2 original_fill_px(ivec2 p) {\n"
	"    if (0 == 0) return original_fill_layer(p, false);\n"
	"    ivec2 q = original_fill_layer(p, true);\n"
	"    // Keep actual auxiliary silhouettes when no foreground cavity surrounds\n"
	"    // them. Only unwritten pixels fall back to repairing auxiliary seams.\n"
	"    if (any(notEqual(q, p)) || texelFetch(originalMaskTex, p, 0).r != 0u) return q;\n"
	"    return original_fill_layer(p, false);\n"
	"}\n"
	"\n"
	"vec3 original_fetch_raw(ivec2 p) {\n"
	"    p = original_fill_px(p);\n"
	"    uint pen = texelFetch(originalIdxTex, p, 0).r & 0x7fffu;\n"
	"    uint w = texelFetch(palTex, ivec2(pen & 255u, pen >> 8), 0).r;\n"
	"    uint r = (w >> 10) & 31u, g = (w >> 5) & 31u, b = w & 31u;\n"
	"    return vec3(float((r << 3) | (r >> 2)) / 255.0,\n"
	"                float((g << 3) | (g >> 2)) / 255.0,\n"
	...
	"}\n"
	"\n"
	"vec3 original_fetch_at(ivec2 p) {\n"
	"    ivec2 sz = textureSize(originalIdxTex, 0);\n"
	"    p = clamp(p, ivec2(0), sz - 1);\n"
	"    ivec2 b = (p / 2) * 2;\n"
	"    if (b.x + 1 < sz.x && b.y + 1 < sz.y) {\n"
	"        bool a = (texelFetch(originalMaskTex, b, 0).r & 3u) == 3u;\n"
	"        bool c = (texelFetch(originalMaskTex, b + ivec2(1, 0), 0).r & 3u) == 3u;\n"
	"        bool d = (texelFetch(originalMaskTex, b + ivec2(0, 1), 0).r & 3u) == 3u;\n"
	"        bool e = (texelFetch(originalMaskTex, b + ivec2(1, 1), 0).r & 3u) == 3u;\n"
	"        // Only a complete, explicitly tagged hardware-dither pair qualifies.\n"
	"        // Real checkerboard artwork and ordinary polygon edges stay untouched.\n"
	"        if (a == e && c == d && a != c)\n"
	"            return 0.25 * (original_fetch_raw(b) + original_fetch_raw(b + ivec2(1, 0))\n"
	"                         + original_fetch_raw(b + ivec2(0, 1)) + original_fetch_raw(b + ivec2(1, 1)));\n"
	"    }\n"
	"    return original_fetch_raw(p);\n"
	"}\n"
	"\n"
	"bool covered(ivec2 p, bool foreground) {\n"
	"    uint tag = texelFetch(maskTex, p, 0).r;\n"
	...
	"}\n"
	"\n"
	"vec3 enhanced_fetch_at(ivec2 p) {\n"
	"    ivec2 sz = textureSize(idxTex, 0);\n"
	"    p = clamp(p, ivec2(0), sz - 1);\n"
	"    ivec2 b = (p / 2) * 2;\n"
	"    if (b.x + 1 < sz.x && b.y + 1 < sz.y) {\n"
